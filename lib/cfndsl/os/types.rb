...
module CfnDsl
  module OS
    module Types
      os_types = YAML.load(File.open("#{File.dirname(__FILE__)}/types.yaml"))
      Types.const_set('Types_Internal', os_types)
      ...
      os_types['Types'].each_key do |typename|
        if !Types.const_defined?(typename)
          klass = Types.const_set(typename, Class.new(Type))
      ...
      classes.each_pair do |typename, type|
        ...
        typeval.each_pair do |attr_name, attr_type|
          if attr_type.is_a?(Array)
            klass = CfnDsl::OS::Types.const_get(attr_type[0])
            ...
            klass = CfnDsl::OS::Types.const_get(attr_type)
            variable = "@#{attr_name}".to_sym
